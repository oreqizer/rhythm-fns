...
test('getBoxMargin', (t) => {
  ...
  t.equal(fns.getBoxMargin(16, 1.5, 2, 0.5), 12, 'round height, fraction margin');
  ...
  t.equal(fns.getBoxMargin(16, 1.5, 3.5, 0.5), 12 + 6, 'fraction height, fraction margin');
...
test('getLineMargin', (t) => {
  ...
  t.equal(fns.getLineMargin(16, 1.5, 1.5), 36, 'larger, fraction - no border');
  t.equal(fns.getLineMargin(16, 1.5, 0.5), 12, 'smaller - no border');
  t.equal(fns.getLineMargin(16, 1.5, 0), 0, 'zero - no border');
  t.equal(fns.getLineMargin(16, 1.5, 2, 2), 46, 'larger, round - border');
  t.equal(fns.getLineMargin(16, 1.5, 1.5, 2), 34, 'larger, fraction - border');
  t.equal(fns.getLineMargin(16, 1.5, 0.5, 2), 10, 'smaller - border');
